...
export function AISettingsPanel({
  ...
  const [activeTab, setActiveTab] = useState<"general" | "course-prompts">("general");
  const [prompts, setPrompts] = useState<CoursePrompt[]>([]);
  const [promptsLoading, setPromptsLoading] = useState(false);
  const [savingPromptId, setSavingPromptId] = useState<string | null>(null);
  const [drafts, setDrafts] = useState<Record<string, Partial<CoursePrompt>>>({});

  const loadPrompts = useCallback(
    async (force = false) => {
      if (!force && prompts.length > 0) return;

      setPromptsLoading(true);
      const { data, error } = await apiGet<CoursePrompt[]>("/api/course-prompts?full=true");
      if (!error && data) {
        setPrompts(data);
        setDrafts(
          Object.fromEntries(
            data.map((p) => [
              p.id,
              {
                ...
                report_structure: p.report_structure,
  ...
  const handleSavePrompt = async (id: string) => {
    ...
    setSavingPromptId(id);
    const { error } = await apiPut<CoursePrompt>(`/api/course-prompts/${id}`, draft);
    if (!error) {
      toast.success("课程提示词保存成功");
      const { data } = await apiGet<CoursePrompt[]>("/api/course-prompts?full=true");
      if (data) {
        setPrompts(data);
        setDrafts(
          Object.fromEntries(
  ...
  return (
    <div className="space-y-6">
      <Tabs
        value={activeTab}
        onValueChange={(value) => {
          const tab = value as "general" | "course-prompts";
          setActiveTab(tab);
          if (tab === "course-prompts") {
            loadPrompts();
          }
        }}
      >
        <TabsList>
          ...
          <TabsTrigger value="course-prompts">课程提示词</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="course-prompts" className="mt-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                            ...
                            <Textarea
                              ...
                              onChange={(e) =>
                                setDrafts((prev) => ({
                                  ...
                                  [prompt.id]: {
                                    ...prev[prompt.id],
                                    report_structure: e.target.value,
                                  },
                                }))
                              }
                              placeholder="输入报告结构..."
