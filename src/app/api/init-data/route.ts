...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (tagsError) {
      ...
      return errorResponse("初始化标签失败", 500);
    }
    ...
    if (themesError) {
      console.error("Failed to insert themes:", themesError);
      return errorResponse("初始化主题失败", 500);
    }
    ...
    if (!existingStages || existingStages.length === 0) {
      ...
      await client.from("course_stages").insert(
        DEFAULT_COURSE_STAGES.map(stage => ({
      ...
      results.courseStages = DEFAULT_COURSE_STAGES.length;
    }

    return successResponse(results, "初始化成功");
  } catch (error) {
